...
.filter('flattenRows', function() {
    return function (blistings) {
        var flatten = [];
        angular.forEach(blistings, function (blisting) {
            ven = blisting.vendor;
            flatten.push(blisting);
            if (ven) {
                angular.forEach(ven, function (vlisting) {
                    vitem = vsliting.item;
                    flatten.push(angular.extend(vlisting, {vlisting: true}));
                        if (vitem) {
                            angular.forEach(vitem, function (ilisting, {ilisting: true}));
                        }
